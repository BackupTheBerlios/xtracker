...
public class ToggleDispositionActiveStateAction extends BaseAction {
	private Logger	logger	= LogManager.getLogger(ToggleDispositionActiveStateAction.class);

	public ToggleDispositionActiveStateAction() {
		super();
	}
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse reponse) throws HibernateException {
		...
		if (!disposition.isImmutable()) {
			disposition.setActive(!disposition.isActive());
